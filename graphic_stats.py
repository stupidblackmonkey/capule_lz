...
class GraphicStatistics:
    def __init__(self, df_path):
        self.df = pd.read_csv(df_path)  # читаем csv

    def histogram(self):
        df = (self.df.groupby("STATE", as_index=False)
              [["FEDERAL_REVENUE", "STATE_REVENUE", "LOCAL_REVENUE"]]
              .mean().sample(8, random_state=1))  # берём 8 штатов

        shtaty = df["STATE"].tolist()  # норм индексы
        znacheniya = df[["FEDERAL_REVENUE", "STATE_REVENUE", "LOCAL_REVENUE"]]
        procenty = znacheniya.div(znacheniya.sum(axis=1), axis=0) * 100  # проценты от общего

        _, ax = plt.subplots(figsize=(18, 10))  # создаём график
        x = range(len(shtaty))
